...
func (s *SubService) BuildURLs(scheme, hostWithPort, subPath, subJsonPath, subClashPath, subId string) (subURL, subJsonURL, subClashURL string) {
	...
	configuredSubURI = safeSubscriptionURI(configuredSubURI)
	configuredSubJsonURI = safeSubscriptionURI(configuredSubJsonURI)
	configuredSubClashURI = safeSubscriptionURI(configuredSubClashURI)
...
// safeSubscriptionURI keeps compatibility with relative and HTTPS URIs while
// refusing stored HTTP URLs unless the operator explicitly enabled the legacy
// insecure HTTP escape hatch. This protects upgraded databases that contain
// old http:// subscription settings and bypass the settings-page validator.
func safeSubscriptionURI(uri string) string {
	if strings.HasPrefix(strings.ToLower(strings.TrimSpace(uri)), "http://") && !network.InsecureHTTPAllowed() {
		return ""
	}
	return uri
}
...
func (s *SubService) getBaseSchemeAndHost(requestScheme, requestHostWithPort string) (string, string) {
	...
	// HTTPS from a direct TLS listener or a reverse proxy is safe to publish.
	if strings.EqualFold(requestScheme, "https") {
		if subDomain == "" {
			return "https", requestHostWithPort
		}
		subPort, _ := s.settingService.GetSubPort()
		return "https", fmt.Sprintf("%s:%d", subDomain, subPort)
	}

	if network.InsecureHTTPAllowed() {
		if subDomain == "" {
	...
	// Without a secure request, only a locally configured and valid TLS pair
	// is enough evidence to construct an HTTPS URL. Otherwise return empty
	// components so BuildURLs does not publish an HTTP link.
	subKeyFile, _ := s.settingService.GetSubKeyFile()
	subCertFile, _ := s.settingService.GetSubCertFile()
	tlsConfig, err := network.LoadTLSConfig(subCertFile, subKeyFile)
...
func (s *SubService) buildSingleURL(configuredURI, baseScheme, baseHostWithPort, basePath, subId string) string {
	...
	if baseScheme == "" || baseHostWithPort == "" {
		return ""
	}
