...
func (s *Server) Start() (err error) {
	...
	tlsConfig, certErr := network.LoadTLSConfig(certFile, keyFile)
	if certErr != nil {
		logger.Error("Subscription TLS disabled because the certificate pair is invalid:", certErr)
	}
	listener, restricted, err := network.NewRestrictedListener(listen, port, tlsConfig)
	if err != nil {
		return err
	}
	if restricted {
		logger.Warningf("Subscription server requested public HTTP on %q:%d; restricting it to loopback. Configure TLS or set %s=true to opt in to insecure HTTP.", listen, port, network.AllowInsecureHTTPEnv)
	}
	if tlsConfig != nil {
