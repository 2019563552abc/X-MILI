...
type AllSetting struct {
	...
	SubEncrypt                  bool   `json:"subEncrypt" form:"subEncrypt"`                                   // Base64-encode subscription responses (not encryption)
...
func (s *AllSetting) CheckValid() error {
	...
	webTLS := s.WebCertFile != "" && s.WebKeyFile != ""
	...
	subTLS := s.SubCertFile != "" && s.SubKeyFile != ""
	...
	if !network.InsecureHTTPAllowed() && !webTLS && !network.IsLoopbackListen(s.WebListen) {
		return common.NewErrorf("public panel HTTP is disabled; configure a valid TLS certificate, bind webListen to 127.0.0.1, or explicitly set %s=true", network.AllowInsecureHTTPEnv)
	}

	if s.SubEnable && !network.InsecureHTTPAllowed() && !subTLS && !network.IsLoopbackListen(s.SubListen) {
		return common.NewErrorf("public subscription HTTP is disabled; configure a valid TLS certificate, bind subListen to 127.0.0.1, or explicitly set %s=true", network.AllowInsecureHTTPEnv)
	}

	if s.SubEnable && !network.InsecureHTTPAllowed() {
		for _, uri := range []string{s.SubURI, s.SubJsonURI, s.SubClashURI} {
			if strings.HasPrefix(strings.ToLower(strings.TrimSpace(uri)), "http://") {
				return common.NewErrorf("subscription URI must use HTTPS; configure TLS or explicitly set %s=true", network.AllowInsecureHTTPEnv)
			}
		}
	}
