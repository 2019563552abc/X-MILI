...
func (s *Server) Start() (err error) {
	...
	if certErr != nil {
		logger.Error("Web TLS disabled because the certificate pair is invalid:", certErr)
	}
	...
	if restricted {
		logger.Warningf("Web server requested public HTTP on %q:%d; restricting it to loopback. Configure TLS or set %s=true to opt in to insecure HTTP.", listen, port, network.AllowInsecureHTTPEnv)
	}
	if tlsConfig != nil {
