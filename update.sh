...
panel_needs_initialization() {
    ...
    [[ -n "$info" ]] && echo "$info" | grep -Eq "bootstrapPending: true|hasDefaultCredential: true"
}

initialize_pending_credentials() {
    if ! panel_needs_initialization; then
        return
    fi

    warn "Initial panel credentials must be configured before login."
    local panel_username="${X_MILI_USERNAME:-}"
    local panel_password="${X_MILI_PASSWORD:-}"
    if [[ -t 0 ]]; then
    ...
    if ! systemctl restart x-ui; then
        fail "Credential setup succeeded but the panel restart failed. Check: journalctl -u x-ui -e --no-pager"
    fi

    log "Initial panel credentials configured."
